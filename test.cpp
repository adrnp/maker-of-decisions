...
int main()
{
	// This just reads in some test gains
	pair<vector<double>, vector<double> > angles_gains = get_gains("testgains72.csv");
	vector<double> angles = angles_gains.first;
	vector<double> gains = angles_gains.second;

	// Compute the shift using the get_bearing function
	double shift = get_bearing(angles, gains);
	cout << "shift is " << shift << "\n";
